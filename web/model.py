...
'''
# sample frame in shape consumed from:
# localhost:5050/api/frame  *or*
# json.loads(r.get('frame').decode('utf8'))
frame = {
...
def parse_frame(frame):
  '''
  Read in a frame as consumed from:
    localhost:5050/api/frame
      or
    json.loads(r.get('frame').decode('utf8'))
...
def load_encoder():
  '''
  Load a pretrained model built with keras on Python 3.7.3 and return to calling agent
  '''
